...
function JornadaLancamento(props) {
  ...
  const [values, onChange] = React.useState({
    ...
    full_date: moment().utc(true).format('YYYY-MM-DD HH:mm:ss'),
  });
  ...
  const validaQR = async e => {
    let i = null; // ou qualquer valor que faça sentido no seu contexto
    let veiculo = props.veiculos;

    for (i = 0; i < veiculo.length; i++) {
      if (veiculo[i].qrcode == e.data) {
        setVeiculoQr(veiculo[i]);
        return true;
      }
    }
    Toast.show({
