...
export default class JornadaController {
  ...
  static async getLancamentosAPI(days) {
    try {
      ...
      if (!isConnected) {
        Toast.show({
          text:
            'Sem conexão com internet! Para ver lançamentos em um período maior que 60 dias há necessidade de rede, conecte seu smartphone em uma rede estável e tente novamente.',
          type: 'warning',
          duration: 13000,
          buttonText: 'Ok',
        });
      ...
      const response = await http.get(
        Routes.api + '/jornada/lancamentos?days=' + days
      );
      if (isEmpty(response)) {
        Store.dispatch(
          setLancamentos({
  ...
  static async syncLancamentosEnviar(service) {
    ...
    try {
      ...
      if (!service) {
        Toast.show({
          text:
            response.data.length + ' Lançamentos Sincronizados com Sucesso!',
          type: 'success',
          duration: 3000,
          buttonText: 'Ok'
        });
  ...
  static async syncLancamentosApi(query) {
    ...
    try {
      const isConnected = await ConnectionController.isConnected();
      if (!isConnected) {
        return Promise.resolve('Sem conexão');
      }
      //OPEN DATABASE
      await DB.open();
      //GET HEADERS
      ...
      const response = await Axios.get(
        Routes.api + '/jornada/lancamentos?days=60',
        axios_config
      );
      if (isEmpty(response)) {
        await DB.close();
