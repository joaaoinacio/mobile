import { Container, Text, Content, Button, ListItem, Icon, View} from 'native-base';
...
function Lancamentos(props){
    ...
    function getLancamentos(startDate, endDate){
        ...
        setTimeout(() => {
            if(moment().diff(moment(startDate), 'days') >= 59){
                console.log('teste')
                JornadaController.getLancamentosAPI(moment().diff(moment(startDate), 'days')).then(()=>{
                    setLoader(false)
                })
                .catch(err => {
                    setLoader(false)
                ...
                JornadaController.getLancamentos(
                    'data > ' + moment(startDate).format('YYYY-MM-DD@HH:mm:ss') +
                    ' AND ' +
                    'data < ' + moment(endDate).format('YYYY-MM-DD@HH:mm:ss')
                ).then(() => {
                    setLoader(false)
                })
                .catch(() => {
    ...
    function onFilterChange(value, name){
        ...
        else getLancamentos(period.startDate, moment(value))
    }
    
    const renderItem = ({item, index}) => <LancamentoItem item={item} index={index} lancamentos={props.lancamentos}/>
    const keyExtractor = (item, index) =>  'jornada_lancamento_item' + index + item.data;
    const emptyComponent = () =>  (
        <View style={styles.emptyContainer}>
            <Text>Sem lançamentos para esse período.</Text>
            <Icon type="FontAwesome" name="ellipsis-h" style={{color: colors.secondary.main}}/>
        </View>
    )
        ...
        <Container>
            ...
            <ScrollView stickyHeaderIndices={[0]}>
                <LancamentoFilter 
                    ...
                    period={period} 
                />
                {loader && !isEmpty(props.lancamentos) && !isEmpty(props.lancamentos.list) && props.lancamentos.list.length > 7 ? <InlineLoader isVisible={loader} dense/> : null}
                <FlatList
