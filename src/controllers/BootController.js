...
class BootController{
    ...
    static async getAppInfo(){
        try{
            ...
            if(userLogs) arrayOfUserLogs = JSON.parse(userLogs)
    ...
    static async syncFetchData(){
        try{
            Store.dispatch(setBootList({
                list: [...Store.getState().bootList.bootList.list, 'Sincronizando dados offline']
            }))
            ...
            if(hasInternet){
                const userLogs = await AsyncStorage.getItem('userLogs')
                console.log(JSON.parse(userLogs))
                if(userLogs){
                    await http.post(Routes.api + '/user-logs', {logs: JSON.parse(userLogs)})
    ...
    static async syncGeolocation(){
        try{
            Store.dispatch(setBootList({
                list: [...Store.getState().bootList.bootList.list, 'Aguardando geolocalizacao']
            }))
    ...
    static async syncAppConfigs(){
        try{
            Store.dispatch(setBootList({
                list: [...Store.getState().bootList.bootList.list, 'Levantando as configurações...']
            }))
